...
- **Peer benchmark follows `docs/benchmark.md`** (spec 0008). The `benchmark-company` task runs after a research run ends `succeeded` (key `benchmark/run/<runId>`), after `updateCompanyFacts` in `src/features/benchmark/actions.ts` (key `benchmark/edit/<companyId>/<updated_at>`) and on `pnpm benchmarks:recompute`; it hands stored rows to the pure `computeBenchmark` in `src/features/benchmark/model.ts` and inserts one immutable `benchmark_snapshots` row (written only by the service role), which the dashboard reads through the version keyed `SNAPSHOT_SCHEMAS` in `snapshot.ts`; no model call in the computation (spec 0012 narrows this: a model proposes peer names and the research pass extracts peer values, both outside `computeBenchmark`, and `tests/features/benchmark/boundary.test.ts` fails if anything under `src/features/benchmark/` imports `src/lib/ai/`). A formula change bumps `MODEL_VERSION` in `src/features/benchmark/catalogue.ts` and adds a schema to the map, never rewrites old rows. Peer values and the seven assumptions live in `supabase/seed-data/*.csv` and reach the database only through `pnpm benchmarks:migration`; every first seed row is `provisional`, and the launch gate in the runbook requires zero provisional rows in production.
- **Named peer comparison follows `docs/benchmark.md`** (spec 0012). Peers are ordinary companies in the seeded house organization (`HOUSE_ORGANIZATION_ID` in `src/features/peers/catalogue.ts`, mirrored by `private.house_organization_id()`), never in a client organization, marked `is_peer` and listed in `peer_companies`; ops propose (one model call, `peer-proposal@1`), approve through `public.approve_peer_company` (advisory lock, ten per section and band, labels `Peer A` to `Peer J`), research through `startPeerRuns` in `src/features/peers/runs.ts` (service client, because an ops token never carries the house organization; the re read, the open run index and the house quota trigger are the guards) and refresh daily through `refresh-peer-companies`. `computeBenchmark` takes the peer values as an argument and adds a `peerSet` block per KPI from five peers on, never for `iso_45001_certified`; every CHF figure and gap rank still comes from the `benchmarks` statistics rows alone. A formula change bumps `MODEL_VERSION` and adds a schema; `benchmark-model@1` rows stay readable.
...
- Tests: Vitest with Testing Library, Playwright with axe (both installed). `axe-core` is a dev dependency too, so a Vitest test can run axe over a rendered state the e2e specs never walk (`tests/features/benchmark/ui/benchmark-segment.test.tsx`).
