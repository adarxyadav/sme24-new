...
- Every emailed link points at `/api/auth/confirm?token_hash=…&type=…&next=…` and is verified from its token hash, so it works in any browser or on a phone. `next` is the absolute destination the action passed as `emailRedirectTo` and carries the locale. A sign in or sign up link carries the bare locale root (`https://…/de`), so the handler keeps the language and lands the user on their role home (a client on `/app`, an expert on `/expert`, ops on `/admin`); only the password reset and invite links name a page (`/reset-password`). No allow list entry is needed for that: the `/**` wildcards in `additional_redirect_urls` already cover the locale root.
