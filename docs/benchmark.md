...
- **No badge since 13 Sep 2026.** The counts render as plain rows of `CalculationFacts` at the top of the "How this is calculated" disclosure, beside the computed on date, the KPIs compared count and the confidence driver; the provenance sentence's "Calculated from" wording is what marks them as calculated. The opportunity card keeps only the lost time count as a clause under the working estimate ("from about 1.8 lost time injuries across 420 employees"). The design gallery mirrors the card so axe scans it.
