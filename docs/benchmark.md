...
Twenty four figures sit in the library on 13 Sep 2026, in two states, and neither has had a person's reading yet:

- **Ten figures (six companies, the first pass)** carry `Claude Fable 5.1 (agent read; owner re-read owed)`: an agent fetched each company's own report page and read the figure off it, which is not a person's reading. The gate cannot see these, because the pair is filled. They are the reason the gate is not the whole protection.
- **Fourteen figures (eight companies, the curation pass)** carry an empty pair, so the gate counts them and the task skips them. This is the safer of the two states and the one to use for any future addition: a figure becomes visible by being read, never by default.

Before the production promotion the owner reads every source page, fills the pair on a row that has none and replaces the agent read name on a row that has one, or empties that pair, so the gate finds no unverified row and no agent read row remains.
...
**What the curation holds (13 Sep 2026).** Fourteen companies, the AC-4 counts met: 8 manufacturers and 6 construction groups, with three LTIFR and three TRIFR figures in C and three LTIFR in F.
...
**The eight rows added on 13 Sep 2026 carry an empty verified pair** and are therefore invisible to the task: the owner reads each source page, enters the date and their own name, and only then does the figure reach a client. Until then a client sees the six agent read figures of the first pass, which themselves still owe the owner's re-reading. Spec 0021 AC-3 allows the empty pair as work in progress; the third launch gate query below is what must read zero before a production promotion, and it is the only check that can see the difference. The pgTAP and Vitest seed suites assert the AC-4 counts and that the pair is whole or empty on every row, never that it is set: a test that demanded zero unverified rows locally would force the curation to claim a reading it has not had.

**Units are the trap, and they differ by sector.** Every figure above is per million hours as printed. Rejected for an unreadable or unconvertible unit: Covestro (DE, RIR 1.70 — the 2024 ESRS page states no denominator at all and the 200 000 hour basis appears only in the 2023 methodology page, so the unit would be an inference across years, and a wrong one shows 8.5), Implenia (CH, 37 accidents per 1 000 full time positions, a third convention that cannot be converted without hours Implenia does not publish), PORR (AT, LTIFR 13.5) and Peab (SE, LTIF4 5.9), both printing a rate with no stated denominator. Bouygues returned 403 on both report PDFs. Voestalpine's LTIFR counts only accidents with more than three sick days, so it is not comparable. Sika returned 403; HOCHTIEF renders its key figures dynamically and the numbers come back blank; Schindler publishes fatalities only.

**No company publishes days lost per incident.** Across all fourteen plus the rejected candidates, not one prints an average days lost per accident. They publish a total day count (VINCI 204 991, Royal BAM 2 117, Wienerberger 5 751.5), or a severity rate on a per 1 000 hours basis (VINCI 0.41, Eiffage 0.43), or days per 1 000 full time equivalents (SFS 96), and the conventions do not convert into one another: Eiffage counts the days of accidents from the last three years, VINCI imputes 365 days per fatality, Wienerberger counts calendar days including weekends, Georg Fischer combines accidents with illness. A `lost_days_per_incident` row would therefore be the curator's own division, not a published figure, so the library holds none. This is a finding about what the market publishes rather than a gap more reading can close, and it leaves the bubble chart of scope row 32 without data for its vertical axis: spec 0021's chart contract assumed a published severity per incident that does not exist. Deciding that axis again (a severity rate per 1 000 hours as its own unit, TRIFR, or a one dimensional ranking) is owed before row 32 is built.
