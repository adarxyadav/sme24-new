...
| Peer bubble chart | `PeerBubbleChart` (`src/features/benchmark/ui/peer-bubble-chart.tsx`) | Client component, hand drawn SVG: LTIFR across, days lost per incident up, headcount as bubble area with a floor, the client in `chart-1`, peers in `chart-2`, a dashed sector line in `chart-3`; one focusable bubble each (`role="button"`) with a tooltip on hover and focus, the SVG a labelled `group` (an `img` role would make the bubbles unreachable), an `sr-only` table with the same figures; no motion. Every number is formatted by the server parent (`ui/peer-chart.tsx`) and passed as a string. Spec 0021, AC-22. |
