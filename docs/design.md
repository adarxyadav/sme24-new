...
| Peer dot strip | `PeerDotStrip` (`src/components/ui/peer-dot-strip.tsx`) | Client component behind `ChartContainer`: one axis per KPI, the industry p25 to p75 shaded (`chart-3`), one dot per named peer (`chart-2`), the company as a larger marker (`chart-1`), the direction of better marked; the drawing is decorative and an `sr-only` table of the same values carries the meaning. Spec 0012. |
