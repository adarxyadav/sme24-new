...
| Peer bubble chart | `PeerBubbleChart` (`src/features/benchmark/ui/peer-bubble-chart.tsx`) | Client component, hand drawn SVG: LTIFR across, days lost per incident up, headcount as bubble area with a floor, the client a solid `chart-1` disc, peers a `chart-3` tint under a `chart-2` hairline, every bubble on an opaque surface disc so the grid never shows through, a dashed sector line in `chart-3`; round ticks from `niceTicks` on the horizontal hairline grid of the Recharts charts (no axis lines), the y title over the plot and the x title under its right edge, each bubble named beside itself in `text-label-12` with a `background` halo (`paint-order: stroke`) so a name over a hairline stays legible, the name flipping left near the right edge and dropping below when it would sit on a placed name; one focusable bubble each (`role="button"`) with the tooltip primitive's surface on hover and focus, flipped below a bubble in the upper half; the SVG a labelled `group` (an `img` role would make the bubbles unreachable), an `sr-only` table with the same figures; no motion. The drawing measures its container and draws in pixels, so the type is 12px at every width rather than scaling with a viewBox. Every number, tick labels included, is formatted by the server parent (`ui/peer-chart.tsx`) and passed as a string. Spec 0021, AC-22; redrawn 2026-09-13. |
