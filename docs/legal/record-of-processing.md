...
| `research_peers` | The peer companies one research run found, their published LTIFR and TRIFR and the source page; no person | Peer comparison (spec 0022) | contract | indefinite | Supabase, Parallel, Anthropic (via the Vercel AI Gateway) | Written by the `research-peers` task alone. A peer is a company, not a natural person, and every row cites a public report. Kept as long as the run it belongs to. |
...
| `assessments` | Which expert assessed which client company against which checklist, the site, the visit date, the state | The on site assessment an order was paid for | contract | account | Supabase | Spec 0019. Belongs to the client organisation and goes with it. Names the expert through `expert_id`, which a fulfilled deletion leaves pointing at the anonymised profile. Clients read the state only, never the draft. |
| `assessment_answers` | Ratings and free text notes an expert typed per checklist item | The findings behind the score and the gap report | contract | account | Supabase | Spec 0019. Belongs to the client organisation and goes with it. The note field is free text and may name people, so the form asks the expert not to record the names of individual workers; no client member reads a row, and a submitted set is frozen for every role. |
