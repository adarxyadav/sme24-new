...
The header (`src/components/marketing-header.tsx`) sticks to the top and is fully transparent until the page scrolls, so whatever the page opens with shows through it and there is no seam in either theme; once the page proper is under it, it takes its bottom hairline over a frosted ground (`bg-background/85` plus a backdrop blur). The landing hero runs up behind the bar (`-mt-16`, given back as padding inside) so its jet ground reaches the top of the viewport, and the bar carries `dark` on that route (`DARK_HERO_ROUTES` in the header, the only page with a dark hero) for as long as that hero is behind it, so the lockup, the links and the controls invert and read on it in light mode too. The swap follows the hero's own bottom edge rather than a fixed offset (`useBarState`), because the frosted ground is only 85% opaque: dropping the inversion while the jet hero was still behind the bar left a black lockup washed out over black. It shows the lockup, then How it works, Expert network and Packages centred with `aria-current="page"` on the active one, then the language switch and sign in; about and contact are reached from the footer. The theme control is not in the header: on desktop it lives in the footer bottom bar, below `md` in the sheet that also holds the links. The footer (`src/features/marketing/ui/marketing-footer.tsx`) shows the signature, the one line site description (`metadata.description`) and the mail address on the left, the Product, Company and, once feature 14 passes links, Legal groups on the right, and a bottom bar with the copyright line (build year, `SITE.legalName`) and the theme control; the language switch lives in the header only, so the page has a single language control (a dropdown button and its menu, not a landmark).
