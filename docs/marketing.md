...
- Tests: `tests/features/marketing/` (schema, action branches, JSON-LD, catalog and site facts, components), `tests/features/enquiries/` (schemas, the workflow action), `tests/trigger/purge-enquiries.test.ts`, `tests/trigger/send-email-enquiry.local.test.ts` (the acknowledgement through the task against Mailpit), `supabase/tests/enquiries.test.sql` (policies, grants, constraints, audit), `e2e/marketing.spec.ts` (pages with axe in both themes, slugs, prefill, the contact thread, the rate limit, the discoverability layer) and `e2e/enquiries.spec.ts` (the ops pages).
...
- The first load JavaScript of a marketing page is about 415 kB gzipped in the production build (the browser Sentry SDK and its React integration, the zod runtime pulled in by the browser env parsing, the Radix primitives of the header and the form). Spec 0009 asks for 120 kB per route; meeting it needs decisions outside this feature (a lighter error client on the public path, env parsing without zod in the browser), recorded for `/architect`.
