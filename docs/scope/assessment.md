...
- [x] Review it (fresh model): `/check review expert accounts & profiles` — reviewed on Claude Sonnet 5 (Opus wrote the code), Approve with nits over 105 files ([review](../reviews/2026-09-08-feat-expert-accounts-profiles.md)); one major (no unit tests for `src/features/experts/actions.ts`, the branch's highest risk authorization file), three minors, three nits, no blockers.
