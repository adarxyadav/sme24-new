...
Superseded on 14 Sep 2026: the owner made the product global rather than Europe country by country, and the peers now come from the research run itself (Slice 11, feature 33, spec 0022). Rows 29, 30 and 32 stop where they are; their open boxes are not built.

### 29. Client country as a benchmark input · superseded by 33
> Superseded on 14 Sep 2026 by [33](#33-peer-benchmark-from-the-research-run--in-progress) (spec 0022): the country is an input of the research run and no per country table exists. Spec 0020 was never built.
...
### 30. Named published peers · superseded by 33
> Superseded on 14 Sep 2026 by [33](#33-peer-benchmark-from-the-research-run--in-progress) (spec 0022): the peers come from the research run, the curated library and its tables are removed. The rules and the card built here (`benchmark-model@5`) are replaced by `@7`; the recompute owed by AC-15 is no longer owed.
...
- [ ] Build it: `/develop named published peers` · code in `src/features/benchmark/` (`model.ts`, `snapshot.ts`, `ui/peer-standing.tsx`), `src/lib/countries.ts`, `src/trigger/benchmark-company.ts`, `supabase/schemas/27_peer_companies.sql` and `28_peer_figures.sql`
  - [x] The thin thread: the whole country catalogue with regions and currencies, the two peer tables with pgTAP, both CSVs with three verified manufacturers, the seed script extension, the task's library read, `benchmark-model@5` with the ladder's happy path and the `peers` block, a first rank card in place of the LTIFR row (AC-1, AC-2, AC-3, AC-5, AC-9)
  - [x] The rules: freshness, latest year, basis preference, the four rungs and the minimum of three, rank ties, the gap to the best, the ISO share, the saving per peer on one arm, the unknown country rule, the unit conversion (AC-6, AC-7, AC-8, AC-13)
  - [x] The card: the strip with its screen reader sentence, both rank line shapes, the rung sentence, the linked rows, the no saving text, the gallery section, both catalogs, the forbidden word test on the new keys, the "no published peer yet" text (AC-10, AC-11, AC-12)
...
### 32. Peer bubble chart · superseded by 33
> Superseded on 14 Sep 2026 by [33](#33-peer-benchmark-from-the-research-run--in-progress) (spec 0022): the branch `feat/peer-bubble-chart` stays unmerged; whether its chart component is reused is the owner's D-chart decision in spec 0022.
The picture the Peer Standing page promised, split out of feature 30 after its cross check (spec 0021 AC-11, owner decision of 13 Sep 2026): one hand drawn SVG under the positions with LTIFR across, lost days per incident up and headcount as bubble area, the client always drawn, a dashed line at the sector median where one exists, a focusable element per bubble with a tooltip on hover and focus, and a table for screen readers. The first `/develop` stopped at the gate because no company prints days lost per accident as an average (ESRS asks for the total), so the spec was amended on 13 Sep 2026: a peer's lost days figure may be the generator's quotient of two numbers printed in one table (total days lost over lost time accidents, both numbers kept on the row and shown in every tooltip), and the snapshot's chart block carries the points themselves under `benchmark-model@6`, which owes a recompute. The contract is spec 0021 AC-18 to AC-25.
**Done when:** a client in manufacturing sees the chart under the positions with at least three published peers that hold both figures; every bubble is reachable by keyboard and named by the screen reader table; a quotient figure shows both printed numbers in its tooltip; the chart is hidden with one sentence when the client or the peers lack a figure; a `@5` snapshot shows the card without the chart until the recompute; axe passes on the client page and the gallery. Gate: Geberit, Symrise and Covestro (section C) print both ingredients in one table, Vinci (section F) too, so the data exists; the owner's sign off of those rows is a task of the build. `from spec 0021`
spec [0021](../specs/0021-named-published-peers/index.md)
- [x] Design it (spec): `/architect peer bubble chart`
- [ ] Build it: `/develop peer bubble chart`
  - [ ] The quotient unit end to end: the fifth unit, the denominator and note columns in the CSV, the seed schema and the generator, the schema file, the migration read by hand, pgTAP and types, Geberit's row first (AC-18, AC-19)
  - [ ] `benchmark-model@6`: the chart block with the client's point and the peer points, the row's published pair and note, the reader's normalisation of `@5`, the runbook and library tests (AC-20)
  - [ ] The chart: the SVG with axes, area, the sector line, tooltips with both printed numbers, focus, the screen reader table, reduced motion, the hide sentence, the gallery section, both catalogs (AC-21, AC-22)
  - [ ] The rows, the sign off, the recompute and the thread: Symrise and Covestro, the four quotient rows with Vinci's note, the owner reads the six pages and signs them off, the pgTAP counts, the `@6` recompute boxes in the runbook, the e2e chart assertions and axe (AC-23, AC-24, AC-25)
- [ ] Verify it: `/check verify peer bubble chart`
- [ ] Test it: `/test peer bubble chart`
- [ ] Review it (fresh model): `/check review peer bubble chart`
- [ ] Document it: `/document peer bubble chart`

## Slice 11: Global, peers from the research

The 14 Sep 2026 decision: the product is global from the start, not Switzerland first, and the client wants the money. The research run that finds a company's figures also finds its peers, anywhere in the world, and the page prices the gap with the owner's own loss table in the client's currency. Nothing curated sits behind the page any more.

### 33. Peer benchmark from the research run · in-progress
A second research task finds up to eight named companies in the client's sector (country, then region, then world) that publish their LTIFR and TRIFR, with the page each figure came from; the benchmark ranks the client among them and prices the client's estimated loss per year (hours lost per incident from the owner's ISO 45004 table, times a cost per working hour, in the client's currency) and the saving at the peer median; three experts matched by sector and country and one recommended package follow. The curated peer library, the Swiss sector statistics, the Swiss cost constants and the launch gate are removed with no fallback, and the country becomes an input of the lookup.
...
- [ ] Build it: `/develop peer benchmark from the research run`
  - [ ] Rails: the world catalogue, the migration (currency, the country on three forms, `research_peers` with pgTAP, expert countries, the photo policy, the snapshot columns, the four tables dropped), the seed code deleted, the catalogue trims (AC-1, AC-2, AC-4, AC-9, AC-19, AC-25)
  - [ ] The peer task: `createPeerRun` on both providers, `research-peers` with its own budget and failure hook, units in code, `peer-validation@1` with the self drop, the rung, the insert, the trigger hand off (AC-3, AC-5 to AC-8, AC-10, AC-11)
  - [ ] The model: `loss.ts`, `benchmark-model@7` with the peers, loss and recommendation blocks, the task loading the run's peers, the raw version and `outdated`, the email copy, the table tests (AC-12 to AC-18)
  - [ ] The page: peers, loss, experts and package in order, `expert_suggestions` with its pgTAP file and the seeded experts, both catalogs (AC-20 to AC-24, AC-26, AC-27)
