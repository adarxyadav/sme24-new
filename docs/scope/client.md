...
spec [0012](../specs/0012-derived-injury-counts.md)
- [x] Design it (spec): `/architect derived injury counts`
- [ ] Build it: `/develop derived injury counts`
  - [ ] Version safety and the column: the literal keyed `SNAPSHOT_SCHEMAS` restructure landed on its own, the nullable `derived jsonb` migration with the pgTAP check, the v2 block schemas and the `MODEL_VERSION` bump, the four pinned test literals moved deliberately (AC-12, AC-13)
  - [ ] The derivation: `exposureCount` lifted out of `costAt` dispatching on rate shape, the derived block in `computeBenchmark` with the Suva fallback and the missing input guards, `hours_per_fte` registered as used, the block carried through the `benchmark-company` parse and insert (AC-1, AC-2, AC-4, AC-6, AC-7, AC-9, AC-10, AC-11, AC-15, AC-16)
  - [ ] The card and its strings: the block in the opportunity card above the CHF figure, lost time then recordable at one decimal, the outline `Calculated` badge with its gallery section, the provenance lines including the Suva variant, both catalogs (AC-1, AC-2, AC-3, AC-5, AC-8)
  - [ ] Harden and document: Vitest over the model and the version map, the Playwright assertion with axe, `docs/benchmark.md` and the post deploy recompute note (AC-13, AC-14)
- [ ] Verify it: `/check verify derived injury counts`
