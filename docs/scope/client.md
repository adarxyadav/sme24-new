...
### 32. Peer bubble chart · in-progress
...
- [ ] Build it: `/develop peer bubble chart` (code in `src/features/benchmark/ui/peer-bubble-chart.tsx`, `ui/peer-chart.tsx`, `ui/chart-scale.ts`, `model.ts`, `snapshot.ts`, `seed-schema.ts`; the owner's sign off of the six rows is the open part)
  - [x] The quotient unit end to end: the fifth unit, the denominator and note columns in the CSV, the seed schema and the generator, the schema file, the migration read by hand, pgTAP and types, Geberit's row first (AC-18, AC-19)
  - [x] `benchmark-model@6`: the chart block with the client's point and the peer points, the row's published pair and note, the reader's normalisation of `@5`, the runbook and library tests (AC-20)
  - [x] The chart: the SVG with axes, area, the sector line, tooltips with both printed numbers, focus, the screen reader table, reduced motion, the hide sentence, the gallery section, both catalogs (AC-21, AC-22)
  - [ ] The rows, the sign off, the recompute and the thread: Symrise and Covestro, the four quotient rows with Vinci's note, the owner reads the six pages and signs them off, the pgTAP counts, the `@6` recompute boxes in the runbook, the e2e chart assertions and axe (AC-23, AC-24, AC-25). Built on 13 Sep 2026 except the sign off: the six rows sit in the CSV with an empty verified pair, the pgTAP counts, the runbook boxes and the e2e are in; the owner reads the six cited pages and fills `verified_at` and `verified_by` with `admin`, then `pnpm benchmarks:migration`, `db:reset`, `test:db`
