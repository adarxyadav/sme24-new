...
- [ ] Build it: `/develop data model`
  - [ ] Tenancy core: `private` helpers, organizations, memberships, `create_organization`, profiles extension, organization claim in the hook, seeded organizations (AC-2, AC-3, AC-6, AC-8)
  - [ ] Policy test harness: pgTAP in `supabase/tests/`, `pnpm test:db`, CI database job, first migration with hand checked grants and regenerated types (AC-1, AC-7, AC-9)
