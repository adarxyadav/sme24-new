...
| 16 | Expert accounts & profiles | Slice 5 | in-progress |
...
- **Expert email change in step with Supabase Auth**: `expert_profiles.email` is a copy taken at invite; an auth email change does not propagate, so ops edit it by hand until an auth hook or an ops edit keeps it in step · from spec 0012
- **Expert photo resizing**: a `sharp` variant task for the profile photo, only if the client card or the ops list gets slow on large uploads · from spec 0012
