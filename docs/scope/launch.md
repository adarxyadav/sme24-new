...
From spec 0012 the gate also grows a peer coverage line once feature 27 lands: how many industry sections and size bands hold five or more approved peers with data fresher than twelve months. A client in an uncovered section still sees the statistics only dashboard, which is correct but is a coverage fact ops should see before go live rather than after.
