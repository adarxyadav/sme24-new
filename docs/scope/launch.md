...
### 25c. Peer data refresh (UVG 2026 & Eurostat) · in-progress
Row 25 gave the benchmark a schema that can say what a peer value came from; this row fills it. The seed still holds one sourced KPI of eight, read from UVG-Statistik 2024 (2022 figures) when the current edition is 2026 (2024 figures), so every CHF figure today measures against a bar that is too lenient. Three "confirmed unreadable" conclusions in `docs/benchmark.md` are wrong because only Swiss sources were searched: Eurostat publishes fatal accident rates, accidents by days lost band and accidents by enterprise size for Switzerland by NACE section, which is the NOGA section. Four owner decisions of 12 Sep 2026 (a peer reference of zero is valid, single class sections keep their labelled comparison, fatalities are compared as a rate at compare time, size bands enter as a scaled estimate on the Suva row) are recorded as an amendment to spec 0016, together with the block plan from `docs/benchmark-handoff.md`. No rebuild: data, docs, catalogs and three small model rules.
**Done when:** every peer row is read from UVG-Statistik 2026 or a named Eurostat table with `provisional false`, `source_key` and `basis` filled; `lost_days_per_incident` and `fatalities` have peer rows and each covered section carries its three size band rows; absenteeism is either seeded or declared `no_source`; the runbook says what is readable and where and its worked example is pinned by a test; `costAt` never yields `NaN`; the disclosure says the CHF figure prices lost time accidents only; and one watched `pnpm benchmarks:recompute` on staging has moved every snapshot to the current `MODEL_VERSION` with both gate queries answering as the runbook expects.
...
- [x] Build it: `/develop peer data refresh` · code in `supabase/seed-data/`, `src/features/benchmark/model.ts`, `src/features/benchmark/snapshot.ts`, `src/features/research/catalogue.ts`, `src/features/benchmark/ui/`, `docs/benchmark.md`
