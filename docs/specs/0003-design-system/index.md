...
- `ThemeToggle` (client component): a three way menu (system, light, dark) with the current choice marked, used in the sidebar user menu and as an icon button in the marketing header. It renders the current choice only after mount (the `next-themes` hydration guard), showing a neutral icon until then, so server and client markup match. Without JavaScript it is absent and the system preference applies.
...
- `LocaleMenuItems`: the two locales as `DropdownMenuSub` items for the user menu, reusing the navigation from `src/i18n/navigation`; the existing `LocaleSwitcher` (a `nav` with two links) stays for the marketing header.
