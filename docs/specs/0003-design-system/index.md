...
- `LocaleMenuItems`: the two locales as `DropdownMenuSub` items for the user menu, reusing the navigation from `src/i18n/navigation`; `LocaleSwitcher` stays for the marketing header; since 2026-09-07 it is a dropdown, a pill shaped trigger the height of a `default` button showing the current language with a chevron, and a menu whose items are real links with a check on the current one.
