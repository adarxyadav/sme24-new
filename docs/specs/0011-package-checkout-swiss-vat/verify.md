...
- [x] Make the render task throw repeatedly → the order stays paid, `pdf_failed_at` is set, the ops alert fires, the email still arrives, and ops can retry the render → AC-10  _(2026-09-08: proven in two halves. In the app, with SELLER_IBAN unset the render refused and the order stayed paid with its invoice and its email; ops saw "Failed" and the retry button dispatched a real run. Exhaustion itself is unreachable from the app, because the dev worker skips retries and the Trigger project's own SELLER_IBAN shadows the local one, so the exhausted-retries hook is proven by unit test instead: `tests/trigger/render-invoice.test.ts` calls it directly and asserts `pdf_failed_at` is stamped on that invoice alone, that no `orders` write happens, and that one `invoice.render_failed` alert is raised keyed `invoice-render-failed/<invoiceId>` with fields the real `opsAlertPayloadSchema` and registry presenter accept. The email needs no assertion in the hook: `confirm-order` triggers the render with `.trigger` and sends the confirmation itself, so the two are independent by construction, and `invoiceAttached` is `false` for the whole slice; a test pins both. **Re-verified 2026-09-08** after this row was first ticked on a false premise. The effects hung on `catchError`, which the SDK runs on *every* uncaught error, not once the retries are gone (`tasks/overview.mdx`: `onFailure` "will only be executed once the task run has exhausted all its retries"; `catchError` "gets called when an uncaught error is thrown" and exists to steer retrying). So the first transient render error already stamped `pdf_failed_at` and fired the ops alert, healing itself only if a later attempt succeeded. The hook is now `onFailure`, matching every sibling task, and the suite pins the hook itself: reverting to `catchError` fails 7 of its 8 tests. **Owed on staging**: the full worker path end to end, a real render failing all 3 attempts so the hook fires in production, the alert landing in Slack, and a successful ops re-render clearing `pdf_failed_at`.)_
