...
- [x] 2026-09-13, owner decision: the derived block left the opportunity card for the "How this is calculated" disclosure. The card keeps the lost time count and the headcount as one clause under the working estimate ("from about 1.8 lost time injuries across 420 employees"); both counts with their provenance sentence, the note that only lost time accidents are priced, the computed on date, the KPIs compared count and the confidence driver render as plain rows at the top of the disclosure (`CalculationFacts` in `src/features/benchmark/ui/benchmark-segment.tsx`), the "Calculated from" wording of the provenance sentence standing in for the badge of AC-3, and the exposure line is dropped because `hours_per_fte` already sits in the assumptions list (AC-11).
- [x] 2026-09-13, owner decision (later the same day): the "How this is calculated" disclosure was removed from the client page entirely, and with it every row `CalculationFacts` had taken from the card (both counts, the provenance sentences, the priced note of the spec 0016 amendment AC-31, the computed on date, the KPIs compared count and the confidence driver). Only the lost time clause under the working estimate remains on screen; AC-2, AC-4, AC-6 and AC-11 describe stored data and the email, not the page. The facts form moved to its own card after the positions.
