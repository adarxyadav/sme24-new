...
- [ ] Photo content sniffing, an accepted risk for now (review of 8 Sep 2026): `uploadExpertPhoto` validates `file.type`, the browser reported MIME type, and never the bytes, so a spoofed content type can put a non image at `<expert_id>/photo.<ext>`. Accepted because the bucket is private, only the owner may write their own prefix, and every read is a 10 minute signed URL rendered in an `<img>`, so the blast radius is one expert's own card. Revisit with the `sharp` task above, which would decode the bytes anyway and is the natural place to refuse a file that is not the image it claims to be.
