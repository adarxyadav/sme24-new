...
- [ ] **Two "at the median" savings on one card** (found in block C, 12 Sep 2026, needs an owner decision). The card's saving repeats the whole formula at the peer median of both priced inputs (spec 0008, AC-18), while a gap's saving prices its one KPI at the median and holds the other at the company's value (`soloSaving`). Until the refresh the two were equal because `lost_days_per_incident` had no peer row; now a company that is already better than the peer on lost days sees a smaller saving on the card (CHF 1 081 000 for the fixture company) than on its accident rate gap (CHF 1 174 000), because the card's reference moves its lost days up to 14.5. Candidate rule: the reference takes the peer median of a priced input only where the company is worse than it, so reaching the median never costs anything; a model change under a `@5` bump, not a copy fix. Recorded in `docs/scope/index.md` as deferred. Until it is decided, the two labels say which figures each moves (`card.savingMedian` "all priced figures", `gaps.saving` "from this KPI alone", both catalogs, the fresh model review of 12 Sep 2026), so the page no longer shows two different numbers under one name.
...
- **AC-30** (C1, C3): `basis` and `source_key` are confirmed on a real card in both languages for a company in section C and one in section F, with the app and a fixture worker running; the distribution position wording changes only if `basis` alone reads as misleading, decided with the copy rather than the code.
  - *Outcome, 12 Sep 2026 (C1).* Confirmed on the local stack with a fixture worker: a section C company at 420 and at 120 FTE met the `250+` and the `50-249` band rows (`Suva Tab. 1.2 · NOGA 10–33 · 250 and more employees · 2024 (nearest year)`, `… · 50 bis 249 Mitarbeitende · 2024 (nächstes Jahr)`), a section F company at 120 FTE met `Suva Tab. 1.2 · NOGA 41–43 · 50 to 249 employees`; every one of the four compared inputs carried its `basis` sentence in the disclosure (`data-peer-basis`, four per card, both languages), the fatality row showed the sector figure and the compared rate (`0.55 per 100 000 employed persons`, `Your count as a rate: 0.00 per 100 000 employed persons`, and the German pair), and the absence row named `Abschnitte B–E`. `e2e/benchmark.spec.ts` asserts the source key, the basis sentence, the compared fatality rate and the refreshed figures (saving at the median CHF 1 081 000, gap saving CHF 1 174 000, point row 48.9).
  - *Decision, 12 Sep 2026 (C3).* The band names and the quartile line stay: they are the model's vocabulary, tested in both catalogs, and the point row already forbids them. What misled was the frame around them, because `basis` lives in the closed disclosure and the section description said "between the peer quartiles" as if the spread were a sample of companies. Copy only, both catalogs: `benchmark.positions.description` now says the comparison is against published sector figures, a spread across the sector's Suva branches scaled to the size band where a ratio exists or one figure where the source publishes one, and points to the disclosure for each row's basis; `benchmark.provisionalNote` no longer claims "the peer values are provisional" (no peer row is, since B6), it says some figures behind the estimate are a provisional first reading, which is true of the four cost assumptions that still raise the flag. No code changed.
...
3. [x] Block C: the card checked in both languages for a section C and a section F company, the lost time only disclosure line, the distribution wording decided with the copy, satisfies **AC-30**, **AC-31**
