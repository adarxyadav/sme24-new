...
- **AC-17** (nothing moves for Switzerland): no recompute is owed by this deploy: a Vitest test computes the fixture company under `benchmark-model@7` with the seven keys and matches the runbook's worked figure (CHF 1 961 000 rounded); existing `@4` rows stay `@4` and render the same; `pnpm benchmarks:recompute` is not run for this feature and `scripts/benchmarks-recompute.mts` needs no change.
...
The `@7` snapshot blocks, relative to `@4`: `inputs` + `country: string`, `countryCovered: boolean`; `cost` becomes `CostBlock & { currency: string } | { skipped: { reason, key? } }`; `gaps[].savingMedianChf` renamed `savingMedian`; `assumptions[]` + `country: string`; `results` and `derived` unchanged.
...
| `benchmark-company` (exists, task) | task | `companyId`, `triggerKind`, `researchRunId?` | one `@7` snapshot with `country` and `currency` | service role | as spec 0008 |
...
- `benchmark_snapshots.currency = currencyOf(benchmark_snapshots.country)` for every `@7` row; a `@1` to `@4` row is `CH` and `CHF` (AC-7).
...
- Happy path: a client looks up a company with country `DE` on the fixture provider, the run succeeds, the `@7` snapshot carries `DE` and `EUR`, the card prices in EUR, the positions read "Compared with Germany's national figures", the email's range is in EUR, verifies **AC-2**, **AC-6**, **AC-7**, **AC-9**, **AC-10**, **AC-11** (Playwright, `TRIGGER_DEV_RUNNING=1`).
...
**Phases**:
1. This release: the four columns with defaults, the two constraint swaps, the seed with `country` on every row, the new assumption keys beside the two old CH money keys, the `@7` model and reader. Old `@4` rows are untouched. A preview built before this release still reads the legacy keys, the old email field names and the old columns and keeps working on the shared staging, except that it treats a `@7` snapshot as an unknown version, as at every earlier bump.
...
3. The model and the task: `country` into `computeBenchmark`, the `(key, country)` then `(key, 'ALL')` resolution, the stored skipped cost, `benchmark-model@7` with its schema, the task's country filters and the two scalar columns, the reader's normalisation, the isolation tests, the fixture figure test, satisfies **AC-6**, **AC-7**, **AC-8**, **AC-17**.
...
**Neutral**:
- `benchmark-model@7` is a schema addition, not a formula change, so old rows need no recompute and stay history.
