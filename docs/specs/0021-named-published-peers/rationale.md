...
The curation call: the six new rows enter with an empty pair, as the runbook asks of every addition since the owner signed off the whole library on 13 Sep 2026, and the owner's reading and sign off of those six pages is a task of the slice rather than a promotion chore, because the task skips an unverified row and the e2e needs at least Geberit's quotient row to draw. The first draft of this amendment had the rows carry the agent read marker; the sign off of the same afternoon made that state a thing of the past, and a marker the runbook no longer describes would have been a third state nobody documents.
