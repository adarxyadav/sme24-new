...
- **AC-13** (the peers block): per rate (`ltifr`, `trifr`) with at least one peer: `rung`, `thin`, `median`, `best`, `rank` (1 plus the number of peers strictly better, so equal values share a rank; null when the client has no value for that rate), `of` (the peer count plus one, the client being one of the compared set), `gapToMedian` and `rows`, each `{ peerName, country, headcount, value, periodYear, sourceUrl, confidence, estimatedLoss }`. `estimatedLoss` is that peer's own yearly loss by the formula of AC-14 with the peer's headcount and the peer's rates (a rate the peer did not publish counts as zero for its term), the same constants and the client's currency; null when the peer's headcount is unknown. It is a total, so it grows with the peer's size (owner decision of 14 Sep 2026, kept until Phillip or a client approves the per 100 employees form, Follow-up). A rate with no peer has no block.
- **AC-14** (the loss block, the owner's table): constants in `src/features/benchmark/loss.ts`: `HOURS_PER_FTE = 1800`, `HOURS_PER_LTI = 769`, `HOURS_PER_RECORDABLE = 201`, `HOURLY_COST = 75`, `FATALITY_COST = 1_200_000`. The client's LTIFR, TRIFR and fatalities are the `company_kpi_current` rows (the newest year per key, chosen per key independently; the year used is recorded per key in `inputs.kpis`). `ltis = ltifr × fte × 1800 ÷ 1 000 000`; `recordables = max(0, trifr − ltifr) × fte × 1800 ÷ 1 000 000` (zero with `trifrMissing: true` when the client has no TRIFR); `fatalities` is the count, zero when missing; `loss = (ltis × 769 + recordables × 201) × 75 + fatalities × 1 200 000`, in `companies.currency`. `atMedian` and `atBest` evaluate the same formula with each rate's peer median or best in place of the client's, for the rates that have a peer block, fatalities held; `savingAtMedian = max(0, loss − atMedian)`, `savingAtBest` likewise, both null when no rate has a peer block. The block is null when `fte` is missing or zero or the client has no LTIFR. Money is stored unrounded and rounded once at display and in the email (`roundMoney`, the current `roundChf` renamed). The hourly cost and the fatality price are never rendered anywhere.
- **AC-15** (the recommendation block): `{ packageKey, reason }` from the standing, first match wins: any fatality above zero or `savingAtMedian` above 250 000 → `retainer` (`fatality`, `large_saving`); no LTIFR → `sms` (`no_figures`); worse than the median on both rates → `compliance` (`both_worse`); worse on one → `sms` (`one_worse`); else → `culture` (`both_better`). A rate without a peer block counts as neither better nor worse. The 250 000 is read in the snapshot currency without conversion, the same known simplification as the 75 and the 1 200 000 (Follow-up).
- **AC-16** (scalars and columns): `benchmark_snapshots` gains `currency text null`, `loss_amount numeric null` and `saving_at_median numeric null`; the CHF named columns stay and are null from `@7` on; `results`, `gaps` and `assumptions` lose their `not null` and are null from `@7` on; the `kpis_compared` check stays at 0 to 8 while `@7` writes 0 to 2 (the rates with a peer block); `confidence` is the minimum over the client rows the loss used, a null confidence counting 1 for a client entered row and 0.5 for a research row; `peer_provisional` is always false.
...
- **AC-20** (peer benchmark first): per rate with a block, a table of the peers (name, country, year, the rate, the peer's own estimated loss in the client's currency under the heading "Estimated loss", a source link that opens the page in a new tab) with the client's own row highlighted in place, one rank sentence ("Your LTIFR ranks n of m published peers in <sector> in <rung>", m from `of`) or, when `thin`, "We found only n published peers for your sector". A rate with no block shows one sentence. Every peer table is labelled as found in public reports by the research, never as verified, and its caption carries one short line: estimated with the same formula, a total that grows with company size. No chart in this spec's contract (D-chart, Follow-up).
...
| page | rank sentence | `peers.<rate>.rank`, `of`, `rung`, the section name from the catalogue |
...
- The loss is null without a headcount or without an LTIFR; the savings are null without a peer block; the recommendation always exists.
...
- Happy path: the fixture company runs through the worker; the page shows five country rung peers, a rank, the loss in CHF, the package per the fixture standing, three experts and the package card, verifies **AC-11**, **AC-13**, **AC-14**, **AC-15**, **AC-20** to **AC-23**, **AC-28**.
...
- Rung: eight peers with two in the country and four in the region land on the region rung with six rows; zero kept peers gives no block and `thin`, verifies **AC-7**.
...
4. **The page (75 min)**: the four sections in order, the peer tables, the loss card and its empty state, `expert_suggestions` with its pgTAP file and the seeded experts, the cards, the package card, both catalogs. Satisfies **AC-20** to **AC-24**, **AC-26**, **AC-27**.
...
- [ ] **D-chart**: the shape is still open (a ranked bar strip, the bubble chart from `feat/peer-bubble-chart` with the two rates, or the owner's sketch with TRIFR up and rank across); whatever the shape, any area or height that shows money shows the peer's own estimated loss of AC-13. Out of this spec's contract; a small amendment when decided.
