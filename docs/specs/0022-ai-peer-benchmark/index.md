...
- **AC-20** (peer benchmark first, one table): one badge (peer count, rung, the rung's name), one rank sentence ("You rank **5 of 6** on LTIFR and **6 of 6** on TRIFR among published peers in <sector> in <rung>", each rank and `of` from `peers.rates`, a rate the client lacks left out of the sentence), then one table in the order of `peers.rows` with the columns company (headcount under the name, "No headcount published" when null), country, year, LTIFR, TRIFR, estimated loss in the client's currency (a dash when null) and a source link that opens the page in a new tab; the client's own row is highlighted in place by its LTIFR with "Your figures" in the source column. One footnote under the table: the peers were found in public reports by the research; each estimated loss uses the same formula as the client's from that company's published headcount and rates, in the client's currency, and is a total that grows with company size; rates are per million hours worked and printed figures per 200 000 hours or per 100 workers are converted. When `thin`, the rank sentence is replaced by "We found only n published peers for your sector". Never the word verified. No chart in this spec's contract (D-chart, Follow-up).
