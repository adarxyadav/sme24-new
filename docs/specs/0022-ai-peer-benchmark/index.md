...
- **AC-20** (peer benchmark first): per rate with a block, a table of the peers (name, country, year, the rate, the client's loss at that rate in the client's currency, a source link that opens the page in a new tab) with the client's own row highlighted in place, one rank sentence ("Your LTIFR ranks n of m published peers in <sector> in <rung>", m from `of`) or, when `thin`, "We found only n published peers for your sector". A rate with no block shows one sentence. Every peer table is labelled as found in public reports by the research, never as verified. No chart in this spec's contract (D-chart, Follow-up).
...
**Key invariants**:
- A peer row always carries a source URL and a rung; a peer never carries a loss of its own.
...
- [ ] **D-chart**: the owner sends a screenshot and decides whether the bubble chart from `feat/peer-bubble-chart` is reused with LTIFR across, TRIFR up and headcount as area, redrawn with the loss on one axis, or dropped. Out of this spec's contract; a small amendment when decided.
