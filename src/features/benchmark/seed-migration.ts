...
export function renderSeedMigration(
  ...
  const header = [
    ...
    "-- Every statement is an upsert, so a rerun changes no row count; the one delete retires",
    "-- peer rows the CSV no longer names, so the CSV is the whole peer table.",
    "",
  ];
