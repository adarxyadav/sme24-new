  ...
  return (
    <ul
      className={[
        ...
        //
        // The first track holds the heading block: the one word trade name at `heading-20` over
        // the full catalogue name at `copy-13` (owner decision of 2026-09-10). `4.75rem` is two
        // lines of that subtitle under a one line name, which is what the longest of them takes in
        // both languages ("Compliance-Check, EHS-System- & Kultur-Snapshot"), so a card whose
        // subtitle runs to one line leaves the second empty rather than pulling the promise below
        // it upwards. That is what puts every promise on the same baseline across the row.
        //
        // It was `3rem` for two lines of a `heading-16` name until the trade names landed; the
        // heading is one line now and the subtitle is the part that wraps.
        full
          ? "grid-rows-[repeat(8,auto)] sm:grid-rows-[repeat(16,auto)] lg:grid-rows-[minmax(4.75rem,auto)_repeat(7,auto)]"
          : // The landing cards also run taller than their content needs, because a price deserves
            ...
            "grid-rows-[repeat(5,auto)] sm:grid-rows-[repeat(10,auto)] lg:grid-rows-[minmax(4.75rem,auto)_auto_auto_minmax(3.5rem,auto)_auto]",
      ].join(" ")}
    >
